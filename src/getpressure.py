...
def get_pressure(station_id: str) -> tuple[float, str]:
    """Get the air pressure at sea level for the given station id.
    ...
    response = requests.get(f"{BASE_URL}/locations/{station_id}", params=params, headers=HEADERS, timeout=TIMEOUT)
...
def get_location(input_city: str) -> tuple[str, float]:
    ...
    for station_id, station_coords in _get_wigos_stations().items():
